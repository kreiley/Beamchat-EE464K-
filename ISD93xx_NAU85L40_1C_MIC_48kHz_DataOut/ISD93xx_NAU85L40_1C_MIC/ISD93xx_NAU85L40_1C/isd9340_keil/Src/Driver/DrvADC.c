...
#include "biq_correction_arb_2x.h"
#include "biq_correction_arb_3x.h"
#include "biq_correction_arb_4x.h"
...
const int OSR[4] = {64, 128, 192, 384};
 
// Lowpass response 6th order Biquad (DF2TSOS)
// Fp1=11200 Hz Fp2=12000 Hz Fs=48000 Hz Ap =0.15dB (passband ripple) )
const int32_t biq_lpf_2x[15] = {
    13084, 
    20028, 
    13084, 
    -31664, 
...
// Lowpass response 6th order Biquad (DF2TSOS)
// Fp1=7467 Hz Fp2=8000 Hz Fs=48000 Hz Ap =0.15dB (passband ripple) )
const int32_t biq_lpf_3x[15] = {
    8635, 
    6870, 
    8635, 
    -62680, 
...
// Lowpass response 6th order Biquad (DF2TSOS)
// Fp1=5600 Hz Fp2=6000 Hz Fs=48000 Hz Ap =0.15dB (passband ripple) )
const int32_t biq_lpf_4x[15] = {
    6957, 
    1062, 
    6957, 
    -78840, 
...
ERRCODE DrvADC_AnaOpen(void)
{
	...
	ANA->LDOPD.PD=LdoFlag & 0x03;  	//Recover LDO setting
	ANA->LDOSET=LdoValue & 0x03;	//Recover LDO setting	  
...
/*----------------------------------------------------------------------------------------------*/
/* Function: DrvADC_Config                                                                        */
/*                                                                                              */
/* Parameters:                                                                                  */
/*    MclkFreq   : HCLK frequency in Hertz                                                      */
/*    SampleRate : Desired ADC sample rate in Hertz                                             */
/*    BiqEn      : Determines whether Biquad filter is selected in ADC path:                    */
/*                 BIQ_OFF : No Biquad                                                          */
/*                 BIQ_ARB2X : Biquad 2x downsample SINC correction filter                      */
/*                 BIQ_LPF2X : Biquad 2x downsample LPF filter                                  */
/*                 BIQ_ARB3X : Biquad 3x downsample SINC correction filter                      */
/*                 BIQ_LPF3X : Biquad 3x downsample LPF filter                                  */
/*                 BIQ_ARB4X : Biquad 4x downsample SINC correction filter                      */
/*                 BIQ_LPF4X : Biquad 4x downsample LPF filter                                  */
/*                 BIQ_ARB   : Biquad highest downsample SINC correction filter                 */
/*                 BIQ_LPF   : Biquad highest downsample LPF filter                             */
/*    MaxSDclk  : Maximum Sigma Delta clock to allow in Hertz. Suggest 40960000 or 61440000     */
/*                                                                                              */
/* Returns:                                                                                     */
/*    E_SUCCESS: ADC has been successfully configured.                                          */
/*    E_FAIL   : No sutiable parameters found for configuration.                                */
/*                                                                                              */
/* Description:                                                                                 */
/*    Open analog block clock for config.                                                       */
/*----------------------------------------------------------------------------------------------*/

int DrvADC_Config(int MclkFreq, int SampleRate, E_BIQ_TYPE BiqEn, int MaxSDclk)
{
    int SRdiv;
    int SDclk;
    int ADCdiv, ADCrem;
    ...
    SRdiv = MclkFreq/SampleRate;
    
    // Find best ADC OSR setting
    if(BiqEn==0){
        found_setting = 0;
        for(i=3; i>=0; i--){
            SDclk = OSR[i] * SampleRate;
            ADCdiv = MclkFreq/SDclk;
            ADCrem =  MclkFreq%SDclk;
            if(SDclk > MaxSDclk)
                continue;
            if(ADCrem != 0)
                continue;
        ...
        // BIQ at fixed Oversample
        found_setting = 0;  
        j = ((BiqEn-1)/2)+2;

        for(i=3; i>=0; i--){
            SDclk = OSR[i] * SampleRate * j;
            ADCdiv = MclkFreq/SDclk;
            ADCrem =  MclkFreq%SDclk;
            if(SDclk > MaxSDclk)
                continue;
    ...
    if(!found_setting){
        ...
        SDADC->EN=0;
        SDADC->CLK_DIV = ADCdiv;
        SDADC->DEC.OSR  = i;
        // For high SD clock rates increase current bias for settling time reduction.
        if(SDclk > 5000000)
			SYS->OSC10K.IBGEN = 1;
		else
			SYS->OSC10K.IBGEN = 0;
		if(BiqEn==0){
            ...
            BIQ->BIQ_CTRL.EN = 0;
            BIQ->BIQ_CTRL.SELPWM = 0;    // BIQ at ADC side
            BIQ->BIQ_CTRL.UPSR = j-1;      
            BIQ->BIQ_CTRL.SR_DIV = (SRdiv / j) - 1;
            BIQ->BIQ_CTRL.PRGCOEF = 1;
            if(BiqEn%2){
                // SINC Correction filter
                if(j==2)
                    biqPtr = (int32_t *)biq_correction_2x;
                else if(j==3)
                    biqPtr = (int32_t *)biq_correction_3x;
                else
                    biqPtr = (int32_t *)biq_correction_4x;
            } else {
                if(j==2)
                    biqPtr = (int32_t *)biq_lpf_2x;
                else if(j==3)
                    biqPtr = (int32_t *)biq_lpf_3x;
                else
                    biqPtr = (int32_t *)biq_lpf_4x;
            }
            
            for (i = 0; i < 15; i++){
                BIQ->BIQ_COEFF[i] = *biqPtr++;
            }
