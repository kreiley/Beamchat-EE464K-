...
{
	//NUC140  setting (LSN: least significant nibble, MSN: most significant nibble)
		I2C0->CON.EI = 1;
		NAU85L40_WriteData(deviceID,0x00,  0x000);   /* Reset all registers */
		Delay(0x200);
	
	  NAU85L40_WriteData(deviceID,0x03, 0x0040); //SYSCLK = MCLKI pin;CODEC CLKC = MCLK; ADC_CLK = MCLKI/2; 

		NAU85L40_WriteData(deviceID,0x04, 0x0001); //default
		...
		NAU85L40_WriteData(deviceID,0x09, 0x6000);
		NAU85L40_WriteData(deviceID,0x0A, 0xf13c);

		NAU85L40_WriteData(deviceID,0x10, 0x0003); //wordlength 16 bit - PCMA
		if (deviceID == 0x1C)
		{
			NAU85L40_WriteData(deviceID,0x12, 0x0000); //turn on DO34 in high impedence during offset time
			NAU85L40_WriteData(deviceID,0x14, 0x800f); //turn on TDM mode only
		}
		else if (deviceID == 0x1D)
		{
			NAU85L40_WriteData(deviceID,0x12, 0x0040); //turn on DO34 in high impedence during offset time and offset time shift = 4 channels * datalength + 1 
		...
		NAU85L40_WriteData(deviceID,0x3A, 0x0002); //OSR = 128; sampling rate = (12.288MHz/2)/(128) = 48kHz	

		NAU85L40_WriteData(deviceID,0x11, 0x0002);		 //default
		
		NAU85L40_WriteData(deviceID,0x13, 0x0000);
		/*----------ALC-------------*/
		NAU85L40_WriteData(deviceID,0x20, 0x4070);		
		NAU85L40_WriteData(deviceID,0x21, 0x700B);
		NAU85L40_WriteData(deviceID,0x22, 0x0022); //disable ALC
		...
		/*----------Digital Gain-----------*/
//		NAU85L40_WriteData(deviceID,0x40, 0x0400);
//		NAU85L40_WriteData(deviceID,0x41, 0x0400);
//		NAU85L40_WriteData(deviceID,0x42, 0x0400);
//		NAU85L40_WriteData(deviceID,0x43, 0x0400);	

//		NAU85L40_WriteData(deviceID,0x40, 0x0478); //+15dB
//		NAU85L40_WriteData(deviceID,0x41, 0x0478); //+15dB
//		NAU85L40_WriteData(deviceID,0x42, 0x0478); //+15dB
//		NAU85L40_WriteData(deviceID,0x43, 0x0478); //+15dB

		NAU85L40_WriteData(deviceID,0x40, 0x04A0); //+20dB
		NAU85L40_WriteData(deviceID,0x41, 0x04A0); //+20dB
		NAU85L40_WriteData(deviceID,0x42, 0x04A0); //+20dB
		NAU85L40_WriteData(deviceID,0x43, 0x04A0); //+20dB
		/*--------------------------------*/
		...
		//NAU85L40_WriteData(deviceID,0x60, 0x0040);		
		...
		/*----------MICBIAS-----------*/
		NAU85L40_WriteData(deviceID,0x67, 0x0D07); //2.8V micbias supply **MATT
		/*--------------------------------*/
		NAU85L40_WriteData(deviceID,0x68, 0x7000);// **MATT
		NAU85L40_WriteData(deviceID,0x69, 0x0099);
		NAU85L40_WriteData(deviceID,0x6A, 0x0099);//**MATT
		/*----------PGA-----------*/
